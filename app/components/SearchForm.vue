<template>
  <div class="space-y-6 bg-white dark:bg-gray-900 rounded-lg p-6 shadow-lg">
    <div>
    ...
    <form @submit.prevent="handleSearch" class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          Music Genres <span class="text-red-500">*</span>
          <span class="text-xs text-gray-500">(Select 1-10 genres)</span>
        </label>
        <USelectMenu
        ...
        >
          <template #label>
            <span v-if="selectedGenres.length === 0" class="text-gray-500">
              Select genres...
            </span>
        ...
        <div v-if="genreError" class="mt-1 text-sm text-red-600">
          {{ genreError }}
        </div>
        <div v-if="selectedGenres.length > 10" class="mt-1 text-sm text-red-600">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          Minimum Followers <span class="text-red-500">*</span>
        </label>
        <UInput
          ...
          :disabled="isLoading"
        />
        <div v-if="followerError" class="mt-1 text-sm text-red-600">
          {{ followerError }}
        </div>
      </div>
      ...
      <div class="pt-4">
        ...
        >
          ...
          {{ isLoading ? 'Searching...' : 'Search Playlists' }}
        </UButton>
      </div>
    </form>

    <!-- Error Message -->
    <div v-if="error" class="p-4 rounded-lg bg-red-50 border border-red-200" data-testid="error-message">
      <div class="flex">
